...
const Comment = ({ commentData }) => {
  ...
  const { topLevelComment, totalReplyCount, canReply } = commentData?.snippet
    ? commentData.snippet
    : {};
  ...
  const {
    ...
    publishedAt,
    updatedAt,
  } = topLevelComment?.snippet ? topLevelComment.snippet : commentData?.snippet;
  ...
  return (
    <div className="flex flex-row justify-between">
      <div className="flex gap-5 py-4">
        {authorProfileImageUrl && (
          <img
            className="size-10 rounded-full items-center cursor-pointer"
            alt="author pic"
